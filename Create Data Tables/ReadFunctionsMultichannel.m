clear all 
close all
BST = load(['D:\Jazmin\MultichannelDataTanks\Cortex\spiketableKS4_July']);
...
n = 1;
for i = 1:size(BST.spiketable,1)
    vals = CSI_P{i,4};  % extract [a b] from cell
    if vals(1) >= 0 && vals(2) >= 0 && BST.spiketable.AuditoryP(i) == 1
        BST.spiketable.SignificanceP(n) = 1;
        n = n + 1;
    % elseif CSI_P.Rango(i,1)<=0 && CSI_P.Rango(i,2)<=0
    %     BST.spiketable.Significance(j) = 1;
    else
        BST.spiketable.SignificanceP(n) = 0;
        n = n + 1;
    end
end
 %% 
...
cd (savingPath);
save('spiketable_Aud_Sig_July.mat', 'spiketable');
save('NormalizedIndexesTable_July.mat', 'NormalizedIndexesTable');
save('SignificanceWindowTable_July.mat', 'SignificanceWindowTable');
%% 
